...
async function upsertUser(claims: any) {
  ...
  // For new users, check if this is the first user
  const allUsers = await storage.getAllUsers();
  const isFirstUser = allUsers.length === 0;
  
  // First user gets admin role, others get viewer by default
...
export const requireRole = (...allowedRoles: Array<"admin" | "editor" | "viewer">): RequestHandler => {
  return async (req, res, next) => {
    ...
    // Attach db user to request for convenience
